...
public class Main {
    public static void main(String[] args) {
        int time;    // 跑两次，第一次跑附加题代码，第二次常规代码
        ...
        for(time=1;time<=2;time++){
            ...
            if(time == 1){
                lexer.loadFile(FilePathConfig.REG_Alloc_PATH);
            }
            else{
            ...
            asmGenerator.run();
            if(time == 1){
                new File(FilePathConfig.REG_ALLOC_ASSEMBLY_PATH);
                asmGenerator.dump(FilePathConfig.REG_ALLOC_ASSEMBLY_PATH);
            }
            else{
